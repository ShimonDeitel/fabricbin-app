...
struct ContentView: View {
    ...
    @State private var editingItem: BoltItem?
    ...
    @ViewBuilder
    private func row(for entry: BoltItem) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.name)
                .font(Theme.headlineFont)
                .foregroundColor(Theme.textPrimary)
            Text("Pattern: \(entry.pattern)  \u{2022}  Bin: \(entry.bin)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
        }
        .padding(.vertical, 4)
...
struct ItemEditView: View {
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Pattern (Gingham)", text: $field1)
                        .focused($focusedField, equals: .field1)
                        .accessibilityIdentifier("field1Field")
                    TextField("Bin (Bin C)", text: $field2)
            ...
            .navigationTitle(mode == .add ? "Add Bolt" : "Edit Bolt")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(name: name, pattern: field1, bin: field2, notes: notes)
        case .edit(var entry):
            entry.name = name
